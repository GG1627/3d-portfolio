...
function App() {
  ...
  const targetPosition = new THREE.Vector3(0, 6.3, 7); // Target camera position
  ...
  const [cameraPosition] = useState(defaultPosition);
  ...
  function startAnimations() {
    ...
    gsap.to(".monitorScreen iframe", {
      ...
      duration: 3.5, // Set the duration of the fade-in
    });
  }

  useEffect(() => {
    if (isFullZoom) {
      // Animate to hide elements when zoomed in
      gsap.to(".info", {
        duration: 1,
        opacity: 0,
      });
    ...
    } else if (!isFullZoom && isHalfZoom) {
      // Animate to show elements when zoomed out
      gsap.to(".info", {
        duration: 0.5,
        opacity: 1,
      });
  ...
  }, [isFullZoom]); // Dependency array to trigger animation on change
      ...
      <Canvas onClick={handleZoomClick} shadows>
        ...
        <Suspense fallback={null}>
          ...
          >
            ...
            <meshStandardMaterial color={isDarkMode ? "#49a9fc" : "#adadad"} />
          </mesh>
