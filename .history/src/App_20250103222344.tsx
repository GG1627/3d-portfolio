...
function App() {
  ...
  // Update stored position continuously
  useEffect(() => {
    if (camera) {
      storedPosition.current.copy(camera.position); // Store the current camera position
    }
  }, [camera.position]); // Runs every time the camera position changes
