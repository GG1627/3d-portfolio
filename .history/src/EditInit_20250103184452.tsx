...
export const EditInit: React.FC<EditInitProps> = ({ isClicked, isLerping }) => {
  ...
  const [storedPosition] = useState(new THREE.Vector3(-200, 100, 110)); // Initial camera position
  ...
  useEffect(() => {
    if (camera.current) {
      ...
      camera.current.position.set(-10, 100, 110);
    }
  }, []);
  ...
  return (
    <>
      <perspectiveCamera ref={camera} position={storedPosition.toArray()} />
    </>
  );
};
