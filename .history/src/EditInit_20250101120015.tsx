...
export default class EditInit {
  ...
  animate() {
    ...
    if (this.isClicked && this.targetPosition) {
      const distance = this.camera.position.distanceTo(this.targetPosition);
      if (distance > 0.1) {
        // Continue lerping if not close enough
        this.camera.position.lerp(this.targetPosition, this.lerpSpeed);
      } else {
        this.isClicked = false; // Stop lerping once close enough
        this.targetPosition = undefined; // Clear the target position
      }
    }
  }
