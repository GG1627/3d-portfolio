...
function App() {
  ...
  const [storedPosition, setStoredPosition] = useState<THREE.Vector3 | null>(
    null
  ); // State to store position before zooming
  ...
  const handleZoomButtonClick = () => {
    if (!isClicked) {
      if (cameraRef.current) {
        setStoredPosition(cameraRef.current.position.clone());
      }
      setIsClicked(true);
    } else if (isClicked) {
      setIsClicked(false);
  ...
  useEffect(() => {
    ...
    } else if (!isClicked && storedPosition) {
      setCameraPosition(storedPosition);
    }
  }, [isClicked]);
