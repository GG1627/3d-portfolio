...
function CameraTracking({
  ...
  const oscillationSpeed = 0.01; // Control the speed of the oscillation
  const oscillationAmplitude = 10; // Control how far left and right the camera moves

  useFrame(({ clock }) => {
    const elapsedTime = clock.getElapsedTime(); // Get the elapsed time in seconds
    const oscillationOffset =
      Math.sin(elapsedTime * oscillationSpeed) * oscillationAmplitude;

    // Apply the oscillation to the camera's x-position
    if (camera && camera.position) {
      camera.position.x = defaultPosition.x + oscillationOffset;
    }
