...
export default class EditInit {
  ...
  storedPosition: THREE.Vector3; // Dynamically store the initial position before zooming in
  ...
  constructor(canvasId: string, isClicked: boolean, isLerping: boolean) {
    ...
    this.storedPosition = new THREE.Vector3(-100, 0, 0); // Default as undefined initially
  ...
  updateStates(isClicked: boolean, isLerping: boolean | undefined) {
    ...
    if (!this.isClicked && this.camera) {
      // Copy the current camera position to storedPosition
