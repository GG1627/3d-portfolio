...
export default class EditInit {
  ...
  animate() {
    ...
    // Handle camera lerping based on isClicked state
    // const target = this.isClicked ? this.targetPosition : this.defaultPosition;
    // this.camera.position.lerp(target, this.lerpSpeed);
    if (this.isLerping) {
      const target = this.isClicked
        ? new THREE.Vector3(0, 20, 50)
        : new THREE.Vector3(-80, 50, 110);
      this.camera.position.lerp(target, this.lerpSpeed);
    }
  }
  ...
  //   updateClickState(isClicked: boolean) {
