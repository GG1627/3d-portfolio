...
function App() {
  ...
  // Set camera position on load and button click
  useEffect(() => {
    if (!camera) return; // Ensure camera is available
    camera.position.copy(defaultPosition); // Set initial position
  }, [camera]);

  useEffect(() => {
    if (isClicked && camera) {
      camera.position.set(0, 0, 9); // Set target position after click
    }
  }, [isClicked, camera]);
  ...
  return (
    <>
      ...
      <Canvas shadows>
        {/* <PerspectiveCamera makeDefault position={[-45, 28, 65]} fov={45} /> */}
